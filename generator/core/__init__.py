...
class NewsletterGenerator:
    ...
    def get_newsletter_graph(self):
        ...
        def generate_newsletter(state):
            prompt = role + """다음 내용들을 확인해서 한글로 뉴스레터를 작성해줘.
            기사 내용: """ + state["article"] + "\n기사 키워드: " + state["keywords"] + "\n기사 요약: " + state["summary"] + "\n뉴스레터 내용:"
            result = llm.invoke(prompt)
            return {"newsletter": result.content, "newsletter_summary": state["summary"]}

        ## articles to newsletter
        ### input : newsletter
        ### output : newsletter_summary
        def summarize_newsletter(state):
            prompt = role + """다음 내용들을 확인해서 뉴스레터를 두 개의 문장으로 요약해줘.
            뉴스레터 내용: """ + state["newsletter"] + "\n뉴스레터 요약:"
            result = llm.invoke(prompt)
            return {"newsletter_summary": result.content}

        ## articles to newsletter title
        ### input : newsletter, newsletter_summary
        ### output : newsletter_title
        def generate_newsletter_title(state):
            prompt = role + """다음 내용들을 확인해서 뉴스레터 제목을 생성해줘.
            뉴스레터 내용: """ + state["newsletter"] + "\n뉴스레터 요약: " + state["newsletter_summary"] + "\n뉴스레터 제목:"
            result = llm.invoke(prompt)
        ...
        graph.add_node("기사 요약", RunnableLambda(summarize_article))
        graph.add_node("뉴스레터 생성", RunnableLambda(generate_newsletter))
        graph.add_node("뉴스레터 요약", RunnableLambda(summarize_newsletter))
        ...
        graph.add_edge("뉴스레터 생성", "뉴스레터 요약")
